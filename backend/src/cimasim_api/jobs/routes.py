...
def preflight(
    ...
    response.headers["Cache-Control"] = "no-store"
    if request.template_id == SKY130_TEMPLATE_ID:
        # request.netlist was already generated by trusted server-side code
        # during JobCreateRequest validation; it is only echoed back here,
        # read-only, for transparency. The free-form parser is never involved.
        netlist = request.netlist or ""
        lines = [
            line.strip()
        ...
        devices = len(lines)
        nodes = {word for line in lines for word in line.split()[1:4] if word != "0"}
        return NetlistPreflightResponse(
            ...
            models=0,
            subcircuits=0,
            outputs=list(request.requested_outputs or []),
            temperature_celsius=(
                27.0 if request.temperature_celsius is None else request.temperature_celsius
            ),
            sandbox_ready=settings.sky130_template_enabled,
            netlist=netlist,
        )
